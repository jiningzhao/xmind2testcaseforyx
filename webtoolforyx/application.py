...
from werkzeugforyx.utils import secure_filename
...
from xmind2testcaseforyx.utils import get_xmind_testsuites, get_xmind_testcase_list
...
def delete_record(filename, record_id):
    ...
    yaxintest_file = join(app.config['UPLOAD_FOLDER'], filename[:-5] + 'xls')

    for f in [xmind_file, testlink_file, zentao_file,yaxintest_file]:
        if exists(f):
            os.remove(f)
...
def delete_records(keep=20):
    ...
    for row in rows:
        ...
        yaxintest_file = join(app.config['UPLOAD_FOLDER'], name[:-5] + 'xls')
...
@app.route('/<filename>/to/yaxintest')
def download_yaxintest_file(filename):
    full_path = join(app.config['UPLOAD_FOLDER'], filename)

    if not exists(full_path):
        abort(404)

    yaxintest_xls_file = xmind_to_yaxin_xls_file(full_path)
    filename = os.path.basename(yaxintest_xls_file) if yaxintest_xls_file else abort(404)
