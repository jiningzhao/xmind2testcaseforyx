...
import logging
from xmind2testcaseforyx.metadata import TestSuite, TestCase, TestStep
...
def gen_testcase_type(topics):
    lables = [topic['label'] for topic in topics]
    lables = filter_empty_or_ignore_element(lables)
    return config['type_sep'].join(lables)
